...
router.post('/', (req, res) => {
    ...
    if (!nom || !prenom || !mail || !mdp || !role) {
        return res.status(400).json({ message: 'Les champs nom, prenom, mail, mdp et role sont obligatoires.' });
    }
...
router.put('/utilisateur/:id', (req, res) => {
    const { id } = req.params; // Récupérer l'ID de l'utilisateur à partir des paramètres de l'URL
    const { nom, prenom, rue, codePostal, ville, pays, tel, mail, dateNaissance, mdp, role } = req.body; // Récupérer les nouvelles valeurs depuis le corps de la requête
    ...
    connexion.query(query, [nom, prenom, rue, codePostal, ville, pays, tel, mail, dateNaissance, mdp, role, id], (error, results) => {
        if (error) {
